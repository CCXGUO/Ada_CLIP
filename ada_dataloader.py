...
class FrameDataset(Dataset):
    def __init__(self, dir1, dir2, transform=None):
        self.dir1 = dir1
        self.dir2 = dir2
        self.transform = transform if transform is not None else self.default_clip_transform()
    ...
    def default_clip_transform(self):
    ...
    def generate_pairs(self):
        ...
        for i, frame in enumerate(self.frames1):
            start= max(0, i - self.total_frames // 40)
            end = min(self.total_frames, i + self.total_frames // 40 + 1)
...
for frameA, frameB, label in dataloader:
    print(f'frameA: {frameA.shape}, frameB: {frameB.shape}, label: {label}')
