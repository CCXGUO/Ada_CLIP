...
def predict(model, img1, img2, device, threshold):
    model.to(device)
    model.eval()
    ...
    for threshold in thresholds:
        ...
        for batch in dataloader:
            ...
            for i in range(len(labels)):
                img1_i = img1[i].unsqueeze(0)  # 添加批次维度
                img2_i = img2[i].unsqueeze(0)  # 添加批次维度

                _, prediction = predict(model, img1_i, img2_i, device, threshold)
...
dir1 = 'C:/Users/17839/dev/Data/AdaClip/validata/Video2'
dir2 = 'C:/Users/17839/dev/Data/AdaClip/validata/Video1'
...
# Initialize the model_weights and other parameters
device = config.device
model = AdaClipNetwork(device)

# 加载保存的模型权重
model_path = 'model_weights/epoch_25.pth'
